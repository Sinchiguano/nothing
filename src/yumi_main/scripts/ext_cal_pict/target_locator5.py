...
def locate_target_orientation(frame):
    ...
    angles_radians=[math.radians(_) for _ in eulerAngles]
...
def main():
    ...
    while(True):
        ...
        cv2.line(frame, tuple(world_imgpts[0].ravel()), tuple(axi_imgpts[1].ravel()), (0,255,0), line_width) #GREEN Y
        cv2.line(frame, tuple(world_imgpts[0].ravel()), tuple(axi_imgpts[0].ravel()), (255,0,), line_width) #BLUE X
        cv2.line(frame, tuple(world_imgpts[0].ravel()), tuple(axi_imgpts[2].ravel()), (0,0,255), line_width) #RED Z
