...
def custom_draw_geometry_with_key_callback(pcd):
    ...
    def load_render_option(vis):
        vis.get_render_option().load_from_json("basic/TestData/renderoption.json")
        return False
...
def custom_draw_geometry_with_camera_trajectory(pcd):
    ...
    custom_draw_geometry_with_camera_trajectory.trajectory =read_pinhole_camera_trajectory("basic/TestData/camera_trajectory.json")
    custom_draw_geometry_with_camera_trajectory.vis = Visualizer()
    if not os.path.exists("basic/TestData/image/"):
        os.makedirs("basic/TestData/image/")
    if not os.path.exists("basic/TestData/depth/"):
        os.makedirs("basic/TestData/depth/")
    def move_forward(vis):
        ...
        if glb.index >= 0:
            ...
            plt.imsave("basic/TestData/depth/{:05d}.png".format(glb.index),\
                    np.asarray(depth), dpi = 1)
            plt.imsave("basic/TestData/image/{:05d}.png".format(glb.index),\
...
if __name__ == "__main__":
    pcd = read_point_cloud("basic/TestData/fragment.ply")
