...
def load_point_clouds(voxel_size = 0.0):
    pcds = []
    for i in range(3):
        pcd = read_point_cloud("basic/TestData/ICP/cloud_bin_%d.pcd" % i)
...
def pairwise_registration(source, target):
    ...
    icp_coarse = registration_icp(source, target,max_correspondence_distance_coarse, np.identity(4),
            TransformationEstimationPointToPlane())
    icp_fine = registration_icp(source, target,max_correspondence_distance_fine, icp_coarse.transformation,
            TransformationEstimationPointToPlane())
    ...
    for source_id in range(n_pcds):
        for target_id in range(source_id + 1, n_pcds):
            ...
            if target_id == source_id + 1: # odometry case
                ...
                pose_graph.edges.append(PoseGraphEdge(source_id, target_id,transformation_icp, information_icp, uncertain = False))
            else: # loop closure case
                pose_graph.edges.append(PoseGraphEdge(source_id, target_id,transformation_icp, information_icp, uncertain = True))
    return pose_graph
