    ...
    while not rospy.is_shutdown():
        ...
        if flag:
            for i, cloud in enumerate(rgbd_):
                ...
                filter = do_passthrough_filter(point_cloud = cloud,name_axis = 'x', min_axis = -0.3, max_axis = 0.4)
                filter = do_passthrough_filter(point_cloud = filter,name_axis = 'y', min_axis = -0.15, max_axis = .25)
